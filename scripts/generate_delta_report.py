"""
Generate DELTA.md comparing invention_batch_001 (before Gap 1 fix)
vs invention_batch_002 (after Gap 1 fix).
...
BATCH_001 = ROOT / "evidence" / "experiments" / "invention_batch_001"
BATCH_002 = ROOT / "evidence" / "experiments" / "invention_batch_002"
DELTA_PATH = BATCH_002 / "DELTA.md"
...
def main():
    ...
    b2_confidences = {}
    for cid in ids:
        b1_file = BATCH_001 / f"{cid}.yaml"
        b2_file = BATCH_002 / f"{cid}.yaml"
        if b1_file.exists():
    ...
    for cid in ids:
        ...
        if c1 is not None and c2 is not None:
            deltas.append({
                "id": cid,
                "batch_001": round(c1, 4),
                "batch_002": round(c2, 4),
                "delta": round(c2 - c1, 4),
    ...
    md = ["# DELTA report — batch_001 (pre-fix) vs batch_002 (post-fix)",
          "",
          f"Date: 2026-08-01",
          f"Gap fixed: Gap 1 (identical scoring)",
          f"Architecture modification: simulation_module.py ONLY (per CEO 'pick one' rule)",
          "",
          "## Headline metrics",
          "",
          f"| Metric | batch_001 (pre-fix) | batch_002 (post-fix) |",
          f"|---|---|---|",
          ...
          "|---|---|---|---|---|"]
    for d in deltas:
        md.append(f"| {d['id']} | {d['batch_001']:.4f} | {d['batch_002']:.4f} | {d['delta']:+.4f} | {d['direction']} |")
    md.append("")

    md.append("## What changed")
    md.append("")
    md.append("- The pre-fix failure mode (11/20 candidates producing composite=0.5777) is GONE.")
    md.append(f"- batch_001 had {b1_unique} unique composites; batch_002 has {b2_unique}.")
    md.append(f"- batch_001 max-shared was {b1_max_shared} candidates on one score; batch_002 max-shared is {b2_max_shared}.")
    md.append("- The composite range expanded: batch_001 was 0.3678-0.6827 (range 0.3149);")
    md.append(f"  batch_002 is {b2_min:.4f}-{b2_max:.4f} (range {b2_max - b2_min:.4f}).")
    ...
    md.append("- Gaps 2-7 are UNCHANGED. Per the CEO 'pick one' rule, only Gap 1 was addressed.")
    md.append("")
    ...
    md.append("batch_003/DELTA.md comparing batch_002 vs batch_003.")
    md.append("")
    ...
    md.append("- tests/test_gap1_fix.py: new test file (10 tests) locking the Gap 1 fix")
    md.append("  contract.")
    md.append("- scripts/run_20_invention_experiment_v2.py: copy of the experiment runner")
    md.append("  that writes to batch_002/ instead of batch_001/. NOT a new module — a")
    ...
    print(f"batch_001 unique: {b1_unique}, batch_002 unique: {b2_unique}")
    print(f"batch_001 max-shared: {b1_max_shared}, batch_002 max-shared: {b2_max_shared}")
