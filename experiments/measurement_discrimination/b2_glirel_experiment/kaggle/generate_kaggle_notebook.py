...
# Install GLiREL and dependencies (DO NOT change package versions per CTO directive)
!pip install -q glirel loguru protobuf sentencepiece 2>&1 | tail -5

# DO NOT downgrade huggingface_hub or transformers
# The local_loader.py bypasses the broken _from_pretrained wrapper
...
# Import the local loader (bypasses broken _from_pretrained)
import sys
...
print(f"Loading {MODEL_ID} via local_loader (bypasses broken _from_pretrained)...")
t0 = time.time()
...
# Get model info from local_loader metadata
...
# GLiREL tokenizes with: re.finditer(r'\w+(?:[-_]\w+)*|\S', text)
# Build token list and char-offset mapping
tokens = []
token_char_starts = []
token_char_ends = []
for match in re.finditer(r'\w+(?:[-_]\w+)*|\S', text):
    tokens.append(match.group())
    token_char_starts.append(match.start())
    token_char_ends.append(match.end())

print(f"Text: {text}")
print(f"Tokens: {tokens}")
print(f"Token char starts: {token_char_starts}")
print(f"Token char ends: {token_char_ends}")

# Find entity token positions (start_tok, end_tok, label)
# "Osteoblasts" = token 0
# "calcium" = token 2, "phosphate" = token 3 → multiword entity = tokens 2-3
# "bone" = token 5, "tissue" = token 6 → multiword entity = tokens 5-6
ner = [
...
# Verify span invariant: source[start:end] == span_text
print("\n--- Span invariant verification ---")
for r in result[:5]:
    head_text = r.get('head_text', '')
    head_pos = r.get('head_pos', [0, 0])
    # head_pos are token positions; convert to char positions
    head_char_start = token_char_starts[head_pos[0]] if head_pos[0] < len(token_char_starts) else -1
    head_char_end = token_char_ends[head_pos[1]-1] if head_pos[1]-1 < len(token_char_ends) else -1
    if head_char_start >= 0 and head_char_end >= 0:
        actual = text[head_char_start:head_char_end]
        match = actual == head_text
        print(f"  head: '{head_text}' vs source[{head_char_start}:{head_char_end}]='{actual}' {'✓' if match else '✗ INVALID_SPAN'}")
""")
...
# Entities in character-offset format
entities_a = [
...
# Convert character-offset entities to GLiREL token-offset NER format
# GLiREL tokenizes with: re.finditer(r'\\w+(?:[-_]\\w+)*|\\S', text)
# NER format: [start_token, end_token, label]
import re

def char_entities_to_token_ner(text, entities):
    \"\"\"Convert character-offset entities to GLiREL token-offset NER format.

    GLiREL expects ner as [[start_tok, end_tok, label], ...] where
    start_tok/end_tok are indices into the token list produced by
    re.finditer(r'\\w+(?:[-_]\\w+)*|\\S', text).
    \"\"\"
    # Tokenize with GLiREL's regex
    token_starts = []
    token_ends = []
    for match in re.finditer(r'\\w+(?:[-_]\\w+)*|\\S', text):
    ...
    for ent in entities:
        ...
        for i, (ts, te) in enumerate(zip(token_starts, token_ends)):
            if ts >= char_start and start_tok is None:
                start_tok = i
            if te <= char_end:
                end_tok = i
...
print(f"Source A NER (token positions): {ner_a}")
print(f"Source B NER (token positions): {ner_b}")
...
try:
    ...
    t0_large = time.time()
    large_model = load_glirel_compatible(LARGE_MODEL_ID, device="cuda")
    t1_large = time.time()
